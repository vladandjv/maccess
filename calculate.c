...
main ()
{
  ...
  char slogovi[9], velslog[6], kljuc[4], strana[4], stackst[4], gustina[4];
  ...
  for (;;)
    {
      ...
      GetField ("MAXIMUM NUMBER OF THE KEYS IN THE PAGE - Page Size:                     ", 10, 1, strana, 10, 3, 3, T, prov3);
      GetField ("MAXIMUM NUMBER OF THE PAGES IN MEMORY - Page Stack Size:               ", 11, 1, stackst, 11, 3, 3, T, prov4);
      Cit = Read ();
      if (Cit == -1 || Cit == -2)
        break;
...
char *field;
{
  ...
  j = atof (intermediate);
  if (j < 4 || j > 254 || fmod (j, 2) != 0)
    {
      message ("The range is from 4 to 254!");
      return (F);
    }
  return (T);
}
...
char *field;
{
  ...
  if (j < 3 || j > 255)
    {
      message ("The range is from 3 to 255!");
      return (F);
    }
  return (T);
}
