  ...
  for (;;)
    {
      ...
      switch (_choice)
        ...
        case 5: _choice = Delete ();
          break;
        case 6: _choice = Deleting ();
          break;
        case 7: _choice = Searching ();
          break;
        case 8: _choice = Find ();
          break;
        case 9: _choice = Next ();
          break;
        case 10: _choice = Prev ();
          break;
        case 11:
...
Loading ()
{
  char input[10], input1[10], beginning[22], duration[22], completion[22];
  long long z, start, end;

  strcpy (input, empty_string (9));
  strcpy (input1, empty_string (9));
  for (;;)
    {
      ...
      strcpy (beginning, time_and_date ());
      ...
      for (z = start; z <= end; z++)
        {
          ...
          strcpy (info->Key, to_left (info->Key, 10));
          ...
          strcpy (completion, time_and_date ());
          strcpy (duration, time_difference (beginning, completion));
          strcpy (duration, vrmuredi (duration));
...
Inserting ()
{
  ...
  info->Deleted = 0;
  strcpy (input, empty_string (10));
  strcpy (info->Surname, empty_string (25));
  strcpy (info->Name, empty_string (20));
  strcpy (info->Remark, empty_string (45));
  for (i = 0;;)
    {
      ...
      if (answer ("Is everything correct? Y/N?") == T)
        {
          strcpy (info->Key, to_left (input, 10));
          SHM_Lock (Lock);
          AddRec (DPtr, &DExt, &TaRecNum, info);
          AddKey (IPtr, &IExt, &TaRecNum, (TaKeyStr *) info->Key);
          SHM_UnLock (Lock);
...
Update ()
{
  ...
  strcpy (info->Key, empty_string (10));
  for (i = 0;;)
    {
VRH:
...
{
  int i;
  char input[11], temp[11], remember[11];
  struct Record *info1;
...
Deleting ()
{
  char input[10], input1[10], temp[11];
  ...
  if ((fp = fopen (DatFName, "rb")) == NULL)
    {
      ...
      MakeIndex (&IPtr, &IExt, IndexFName, KeyLen, Duplicates, (long long) 0, (long long) 1);
      CloseFile (DPtr, &DExt);
      CloseIndex (IPtr, &IExt);
      TermAccess ((long long) 0);
      Destroy_SHM (MACCESS_SHM_MEM_CODE, (size_t) MACCESS_SHM_MEM_SIZE);
