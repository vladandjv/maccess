/**************************************************************************/
/* PROGRAM: MACCESS.C                                                     */
/* DESCRIPTION:  Basic modul; basic functions                             */
...
static long long HadBits; /* It used to communicate between TaGetPage as one side 
...
void *Buff;
{
	...
	if (read((*DatE).FileNumber, Buff, (*DatF).Header.ItemSize) == -1)
	{
		IOStatus = ErrGetRec;
		TAIOCheck();
	}
...
void *Buff;
{
	...
	if (write((*DatE).FileNumber, Buff, (*DatF).Header.ItemSize) == -1)
	{
		IOStatus = ErrPutRec;
		TAIOCheck();
	}
	...
	if (Found == F)
	{
		...
		if (HadBits == (*IdxE).Path[(*IdxE).PP].HasBits)
		{
			...
			if ((*TaPageStk[i]).Updated == T)
			{
				...
				TaPutRec((*TaPageStk)[i].IndexFPtr,
					 &((*(*TaPageStk)[i].IndexEPtr).DataE),
					 (*TaPageStk[i]).PageRef,
					 &((*TaPageStk)[i].Page));
			}
			((*TaPageStk)[i]) = *TempPagPtr;
...
void *Buff;
{
	...
	if (write((*DatF).FileNumber, Buff, (*DatF).Header.ItemSize) == -1)
	{
		IOStatus = ErrWriteRec;
		TAIOCheck();
	}
...
void *ProcKey;
{
	...
	if ((*IdxE).PP != -1)
	{
		...
		{
			(*IdxE).Path[(*IdxE).PP].ItemArrIndex++;
			strcpy(ProcKey, (*PagPtr).ItemArray
					    [(*IdxE).Path[(*IdxE).PP].ItemArrIndex]
						.Key);
...
void *ProcKey;
{
	...
	if (OKAY != T && (*IdxE).PP > -1)
	{
		while ((*IdxE).PP > 0 && (*IdxE).Path[(*IdxE).PP].ItemArrIndex == -1)
			(*IdxE).PP--;
		if ((*IdxE).Path[(*IdxE).PP].ItemArrIndex == -1)
			(*IdxE).PP = -1;
	}
...
void *ProcKey;
{
	...
	*DataRecNum = 0;
	...
	if (OKAY != T && (*IdxE).AllowDuplKeys == T)
	{
		...
		NextKey(IdxF, IdxE, DataRecNum, ProcKey);
		if (!(OKAY == T && strcmp(ProcKey, TempKey) == 0))
			OKAY = F;
	}
}
/**************************************************************************/
