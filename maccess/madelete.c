...
static Boolean PageTooSmall;
static TaPagePtr PagPtrAB; /* communication between DelB i DelA */
static long long k;        /* communication betwen DelB i DelA */
/**************************************************************************/
void
  ...
  else
  {
    ...
    if (l - RBr > 1)
    {
      *DataRecNum = (*PagPtrAB).ItemArray[k].DataRef;
      ...
      {
        DelA(XPageRef, PrPgRef, IdxF, IdxE, DataRecNum);
        if (PageTooSmall == T)
...
/**************************************************************************/
void
    DelA(PrPgRef2, PrPgRef, IdxF, IdxE, DataRecNum)
